<template>
  <section
    class="h-96 sm:ml-4 sm:mr-8 mb-8 p-6 bg-sky-100 rounded-2xl shadow-md flex flex-col justify-center items-center"
  >
  ...
  const datasets = [
    {
      ...
      data: data.map((entry: ChartDataEntry) => entry.cobrancaACadaXDias)
    },
    {
    ...
    },
    {
      ...
      data: data.map((entry: ChartDataEntry) => entry.valor)
    }
  ]
